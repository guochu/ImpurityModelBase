...
@testset "Interacting fermions: imaginary time" begin
	...
	for U in (0, 1)
		for ϵ_d in (-0.5, 0., 0.7)
			...
			H, a, adag, Himp, Hbath = interacting_operators(U, ϵ_d, ω₀=1, α=0.5, d=100)
	...
	for U in (0, 1)
		for ϵ_d in (-0.5, 0., 0.7)
			for init_state in (:globalthermal, :localthermal)
				...
				g1 = [independentbosons_greater(spectrum_func(), tj, β=β, ϵ_d=-ϵ_d, U=U, bands=2, init_state=init_state) for tj in 0:δt:t]
				g2 = [independentbosons_lesser(spectrum_func(), tj, β=β, ϵ_d=-ϵ_d, U=U, bands=2, init_state=init_state) for tj in 0:δt:t]

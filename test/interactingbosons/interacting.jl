...
@testset "Interacting fermions: imaginary time" begin
	...
	for U in (0, 1)
		for ϵ_d in (-0.5, 0., 0.7)
			g1 = independentbosons_Gτ(spectrum_func(), β=β, ϵ_d=-ϵ_d, N=N, U=U, nbands=2)
...
@testset "Interacting fermions: real time" begin
	...
	for U in (0, 1)
		for ϵ_d in (-0.5, 0., 0.7)
			g1 = [independentbosons_greater(spectrum_func(), tj, β=β, ϵ_d=-ϵ_d, U=U, nbands=2) for tj in 0:δt:t]
			g2 = [independentbosons_lesser(spectrum_func(), tj, β=β, ϵ_d=-ϵ_d, U=U, nbands=2) for tj in 0:δt:t]

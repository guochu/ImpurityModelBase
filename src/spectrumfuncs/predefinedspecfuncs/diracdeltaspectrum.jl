...
# """
# struct DiracDelta <: AbstractDiscreteFunction
# 	ω::Float64
# 	α::Float64
# 	lb::Float64
# 	ub::Float64
...
function DiracDelta(ω::Real; α::Real=1) 
	return DiracDelta(convert(Float64, ω), convert(Float64, α))
end
DiracDelta(;ω::Real=0, α::Real=1) = DiracDelta(ω, α=α)

lowerbound(x::DiracDelta) = -Inf
upperbound(x::DiracDelta) = Inf

Base.similar(x::DiracDelta, ω::Real; α::Real=x.α) = DiracDelta(ω, α=α)
Base.similar(x::DiracDelta; ω::Real=x.ω, α::Real=x.α) = DiracDelta(ω, α=α)
